...
- `background.ts`의 `runCrawl()`은 `finally`에서 `crawling`만 끄고 `recording`은 끄지
  ...
  다음 세션에 그대로 남는다
- **캡처(관측) 범위와 크롤(자동 탐색) 범위는 의도적으로 다르다.** `popup.ts`는
  "녹화 시작" 시 입력된 도메인 패턴이 아니라 항상 `<all_urls>`를 요청한다(v0.3.0) —
  화이트리스트가 안전하려면 대상 앱이 의존하는 서드파티(CDN·인증·애널리틱스)까지
  빠짐없이 잡아야 하는데, 캡처를 입력 패턴으로 제한하면 그런 서드파티가 조용히
  빠진다(재현율 우선 위반). "내 서비스 도메인" 입력란은 더 이상 권한 요청 범위가
  아니라 `classify`(WASM)에 넘길 1st/3rd-party 라벨링용 힌트일 뿐이며 비워도 동작한다
  (`internal/classify.Apply`는 primaryDomains가 비면 party를 "unknown"으로 표시,
  에러 아님). 반면 **자동 크롤의 탐색 범위(`hostOf(clean) !== seedHost`)는 그대로
  시작점과 같은 호스트로 제한** — 크롤러가 인증된 세션으로 자동 네비게이션하는
  거라, 관측 권한이 넓어졌다고 엉뚱한 도메인까지 자동으로 돌아다니면 안 되기
  때문. `scripts/screenshots`처럼 권한 프롬프트를 우회해야 하는 자동화 도구는 이제
  스테이징 매니페스트의 `host_permissions`를 테스트 서버 origin이 아니라
  `<all_urls>`로 패치해야 한다(요청하는 패턴과 정확히 일치해야 프롬프트 없이
  auto-grant됨)
