...
않는다. SPA 해시 라우트(`#/...`)도 별개 페이지로 따라간다(사이드 내비게이션이
해시 라우팅인 대시보드형 앱 대응 — v0.2.1까지는 프래그먼트를 스트립해 이런 앱에서
시작 페이지 밖으로 못 나가는 버그가 있었다). iframe 내부·웹 컴포넌트(open shadow
root) 메뉴의 링크도 인식한다.
...
- **Phase 7** (진행 중): Chrome 확장 — Go 코어 WASM 재사용, `chrome.webRequest` 수동 캡처로
  ...
  메뉴의 링크 미인식 + 비동기 렌더되는 메뉴에 고정 지연이 부족할 수 있음. 남은 건
