...
- **헤드리스 브라우저 캡처(chromedp)** — 대상 URL을 실제 구동하며 페이지 로드·XHR/fetch·서드파티(CDN/폰트/애널리틱스) 요청 전량 기록. 같은 호스트 링크 자동 크롤(`--depth`, SPA 해시 라우트 `#/...` 포함, 같은 출처 iframe·open shadow root 내부 링크도 인식), 로그인 세션 주입(`--cookie`/`--header`)으로 인증 페이지도 무인 수집. 링크 목록은 `--wait`만큼 대기한 뒤 두 번 연속 같은 개수가 나올 때까지 재확인해 비동기로 늦게 뜨는 사이드 메뉴도 놓치지 않는다. SPA는 여러 진입 URL(`--url` 반복 / `--url-file`)로 라우트를 열거해 각 화면의 API 호출 캡처. 쿠키 주입 시 중복 로그인으로 세션이 만료되는 앱은 `--headful`로 창에서 직접 로그인 후 캡처
...
사용법: 팝업에서 (선택) 내 서비스 도메인 입력 → 녹화 시작 → 대상 앱을 평소처럼
사용 → 정지 → Result JSON/HAR/CSV 내보내기. Result JSON은 CLI의 `extract` 출력과
완전히 동일한 스키마이므로 그대로 파이프 가능. 녹화 중 팝업·정책 검토 페이지 자신을
여닫아도 그 페이지의 리소스 요청(`chrome-extension://...`)은 자동으로 캡처에서
제외된다 — 대상 앱의 트래픽만 남는다.

**캡처 범위는 도메인으로 제한하지 않는다(v0.3.0)**: "녹화 시작"을 누르면 항상
`<all_urls>` 권한을 요청한다(기본으로 부여되지 않고, 클릭 시점에만 요청 — 언제든
`chrome://extensions`에서 취소 가능). 화이트리스트 정책은 대상 앱이 의존하는
서드파티(CDN·인증·애널리틱스) 도메인까지 빠짐없이 포함해야 안전한데(CLAUDE.md
재현율 우선 원칙), 캡처를 입력한 도메인으로 제한하면 정작 그런 서드파티가 조용히
빠지기 때문이다. "내 서비스 도메인" 입력란은 이제 캡처 범위가 아니라 내보내기 시
1st/3rd-party 라벨링에만 쓰인다(비워둬도 동작하며, 이 경우 party는 "unknown"으로
표시). 자동 크롤의 **탐색 범위(어디로 자동 이동할지)는 그대로 시작점과 같은 호스트로
제한** — 이건 관측 범위와 별개로 유지된다(아래).
...
않는다. SPA 해시 라우트(`#/...`, `#!/...`)도 별개 페이지로 따라가되 순수 인앵커
(`#section`)는 같은 페이지로 취급 — CLI의 `normalizeLink()`와 동일한 휴리스틱. iframe
내부·웹 컴포넌트(open shadow root) 메뉴의 링크도 인식하고, 비동기로 늦게 뜨는 메뉴는
링크 개수가 두 번 연속 같아질 때까지 재확인한다 — CLI 쪽 크롤러(`browser.go`)도
v0.2.2에서 동일하게 iframe/shadow DOM 인식과 링크 안정화 대기를 갖췄다(양쪽 캡처
계층이 독립 구현이라 서로 자동으로 맞춰지지 않으므로, 한쪽에 크롤 관련 개선이
들어가면 다른 쪽도 확인해서 맞출 것). 크롤이 끝나도(큐 소진·최대 페이지 도달) 녹화는
자동으로 멈추지 않는다 — 이어서 대상 앱을 수동으로 몇 군데 더 눌러보고 싶을 수 있어서
의도적으로 유지하며, 대신 상태 표시가 "크롤 완료(N페이지) · 녹화 중"으로 바뀌어
크롤이 끝났음을 알려준다.
...
- **Phase 7** (진행 중): Chrome 확장 — Go 코어 WASM 재사용, `chrome.webRequest` 수동 캡처로
  ...
  webRequest 리스너 등록 실패, 크롤 탭 로드 대기 race condition) → v0.2.0. v0.2.0
  실사용 중 자동 크롤 관련 버그 3건 추가 발견·수정 → v0.2.1: (1) 팝업 폼 상태(도메인
  텍스트만, 자동 크롤 체크·depth·최대 페이지 미저장)가 팝업을 닫으면 초기화, (2) 해시
  라우팅 사이드 내비게이션 앱에서 크롤이 시작 페이지 밖으로 못 나감(프래그먼트를
  스트립해 모든 nav 링크가 seed URL과 동일 문자열로 뭉개짐), (3) iframe/웹 컴포넌트
  메뉴의 링크 미인식 + 비동기 렌더되는 메뉴에 고정 지연이 부족할 수 있음. v0.2.1에서
  확장의 해시 처리(2)가 CLI의 기존 `normalizeLink()` 휴리스틱(SPA 라우트만 보존,
  인앵커는 제거)보다 거칠어진 걸 확인해 양쪽을 서로 맞춤(v0.2.2) — 확장은 CLI와 같은
  휴리스틱으로, CLI(`browser.go`)는 확장에서만 있던 iframe/shadow DOM 인식과 링크
  안정화 대기(`waitForStableLinks`)를 갖추도록 포팅. 캡처 계층이 두 곳에 독립
  구현돼 있어 크롤 관련 개선은 앞으로도 양쪽을 매번 대조해서 맞춰야 함. 실사용
  스크린샷으로 크롤이 자연 종료된 뒤에도 팝업이 "녹화 중"만 보여줘 완료 여부를 알 수
  없다는 점 발견 → v0.2.3: `crawlCompleted` 상태를 추가해 "크롤 완료(N페이지) · 녹화
  중"으로 구분 표시(녹화 자체는 의도적으로 유지 — 이어서 수동으로 더 눌러볼 수 있음).
  "대상 도메인으로 캡처를 제한하는 게 이 프로젝트 목적에 맞나?"라는 질문에서
  재검토 → v0.3.0: 캡처 권한을 입력 도메인 패턴 대신 `<all_urls>`로 요청하도록 변경
  (여전히 클릭 시점에만, 기본 부여 없음) — 화이트리스트는 대상 앱이 의존하는
  서드파티까지 빠짐없이 잡아야 안전한데 도메인으로 캡처를 제한하면 그게 조용히
  빠지기 때문(재현율 우선). 도메인 입력란은 1st/3rd-party 라벨링 전용으로 역할이
  바뀌었고, 자동 크롤의 탐색 범위(시작점과 같은 호스트만)는 안전을 위해 그대로 둠.
